...
matchtype = {
    ...
    7: "Others",
    8: "Womens's Tests",
    9: "Womens's One-Day Internationals",
    10: "Womens's Twenty20 Internationals",
    ...
    30: "Womens's Youth One-Day Internationals",
    31: "Womens's Youth Twenty20 Internationals",
    ...
    43: "Womens International The Hundred",
    44: "Womens Domestic The hundred",
    45: "Youth Womens T20",
    50: "T10",
    51: "W T10",
...
BowlingGroup = {1: "RAP", 2: "LAP", 3: "RAO", 4: "SLAO", 5: "RALB", 6: "LAC"}
...
def main():
    ...
    from_date = st.sidebar.date_input('From Date', value=pd.to_datetime('01/01/2000', format='%d/%m/%Y'))
    to_date = st.sidebar.date_input('To Date', value=pd.to_datetime('31/12/2024', format='%d/%m/%Y'))
    selected_Dates = data[(data['Date'] >= from_date) & (data['Date'] <= to_date)]['Date'].unique()

    match_format_ids = list(matchtype.keys())
    selected_match_format_ids = st.sidebar.multiselect("Select match format:", match_format_ids, default=[3])
    selected_match_formats = [matchtype[i] for i in selected_match_format_ids]

    competitions = ['All'] + list(data['CompName'].unique())
    selected_competition = st.sidebar.multiselect("Select competition:", competitions, default=['All'])

    bat_club_names = ['All'] + list(data['BatClubID'].unique())
    selected_bat_club_name = st.sidebar.multiselect("Select the batsman's club name:", bat_club_names, default=['All'])

    if selected_bat_club_name and 'All' not in selected_bat_club_name:
        batsman_names = data[data['BatClubID'].isin(selected_bat_club_name)]['StrikerName'].unique()
    else:
        batsman_names = data['StrikerName'].unique()
        
    selected_batsman_name = st.sidebar.multiselect("Select the batsman's name:", ['All'] + list(batsman_names), default=['All'])

    if selected_batsman_name:
        spin_or_pace = st.sidebar.multiselect("Choose bowler type", ['Pace', 'Spin', 'Both'])

        pace_type = st.sidebar.multiselect("Select pace type:", list(BowlingGroup.keys()), default=[1, 2])
        spin_type = st.sidebar.multiselect("Select spin type:", list(BowlingGroup.keys()), default=[3, 4, 5, 6])

        specific_runs = st.sidebar.multiselect("Select run types:", ['0s', '1s', '2s', '4s', '6s'], default=['0s', '1s', '2s', '4s', '6s'])

        overs_phase = st.sidebar.multiselect("Select overs phase:", ['All', 'Power Play (1-6)', 'Middle Overs (7-15)', 'Death Overs (16-20)'], default=['All'])
        ...
        if 'All' in selected_batsman_name:
            if 'All' in selected_bat_club_name:
                ...
                batsmen_to_plot = data[data['BatClubID'].isin(selected_bat_club_name)]['StrikerName'].unique()
        else:
        ...
        for batsman in batsmen_to_plot:
            ...
            if len(selected_match_formats) > 0:
                filtered_data = filtered_data[filtered_data['MatchTypeID'].isin(selected_match_format_ids)]
            ...
            if 'All' not in selected_bat_club_name:
                filtered_data = filtered_data[filtered_data['BatClubID'].isin(selected_bat_club_name)]
            
            if 'Pace' in spin_or_pace:
                if 'All' in pace_type:
                    filtered_data = filtered_data[(filtered_data['StrikerName'] == batsman) & (filtered_data['BowlingTypeGroup'].isin([1, 2]))]
                else:
                    filtered_data = filtered_data[(filtered_data['StrikerName'] == batsman) & (filtered_data['BowlingTypeGroup'].isin(pace_type))]
            elif 'Spin' in spin_or_pace:
                if 'All' in spin_type:
                    filtered_data = filtered_data[(filtered_data['StrikerName'] == batsman) & (filtered_data['BowlingTypeGroup'].isin([3, 4, 5, 6]))]
                else:
                    filtered_data = filtered_data[(filtered_data['StrikerName'] == batsman) & (filtered_data['BowlingTypeGroup'].isin(spin_type))]
            else:
            ...
            if not filtered_data.empty:
                ...
                for i in range(len(filtered_data)):
                    pitch_x, pitch_y, point_color = calculate_pitch_map_coordinates(
                        filtered_data['LengthX'].iloc[i], filtered_data['LengthY'].iloc[i], origin_x, origin_y, 
